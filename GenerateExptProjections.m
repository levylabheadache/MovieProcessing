...
if nargin > 3  %~exist(projParamPath, 'file') % || projParam.overwrite
    runsDir = sprintf('%sRuns\\',projParam.dir); 
    mkdir(runsDir);
    if expt.Nplane == 1
       projParam.z = {1}; 
    end
    ...
    for t = 1:projParam.Ntype
        if exist(expt.sbx.(projParam.sbx_type{t}), 'file')
            ...
            for runs = 1:expt.Nruns
                ...
                if expt.Nplane == 1
                    [~, tempRunProj(runs,:,1), runBinLims{runs}, tempRawProjPath] = WriteSbxPlaneTif(expt.sbx.(projParam.sbx_type{t}), ...
                        catInfo, 1, ...
                        'chan','both', ...
                        'firstScan',expt.scanLims(runs)+projParam.bin+1, ...
                        'Nscan',expt.Nscan(runs)-projParam.bin, ...
                        'edge',projParam.edge, ...
                        'scale',projParam.scaleFactor, ...
                        'binT',projParam.bin, ...
                        'sbxType',projParam.sbx_type{t}, ...
                        'RGB',false, ...
                        'dir',runsDir, ...
                        'name',tempName, ...
                        'overwrite',projParam.overwrite ); % , 'sbxType',projParam.sbx_type{t}
                    projParam.path.run.raw.z(runs,:,1) = tempRawProjPath;
                else
                    
                    % Z projections
                    if exist(expt.sbx.(projParam.sbx_type{t}), 'file')
                        [tempProj, runBinLims{runs}, tempProjPath] = WriteSbxZproj(expt.sbx.(projParam.sbx_type{t}), ...
                            catInfo, ...
                            'z',projParam.z, ...
                            'chan','both', ...
                            'dir',runsDir, ...
                            'name',tempName, ...
                            'sbxType',projParam.sbx_type{t}, ...
                            'projType',projParam.type, ...
                            'monochrome',true, ...
                            'RGB',true, ...
                            'firstScan',expt.scanLims(runs)+1, ...
                            'Nscan', expt.Nscan(runs), ...
                            'edge',projParam.edge, ...
                            'scale',projParam.scaleFactor, ...
                            'binT',projParam.bin, ...
                            'overwrite',projParam.overwrite); % expt.Nscan(runs)
                    ...
                    if projParam.vol
                        if exist(expt.sbx.(projParam.sbx_type{t}), 'file')
                            [temp_vol{runs}, ~, projParam.path.run.(projParam.sbx_type{t}).vol(runs,:)] = WriteSbxVolumeTif(expt.sbx.(projParam.sbx_type{t}), catInfo, 'chan','both', 'dir',runsDir, 'name',tempName, 'type',projParam.sbx_type{t}, 'monochrome',true, 'RGB',true,...
                                ...
                                'Nscan', expt.Nscan(runs), ...
                                'edge',projParam.edge, ...
                                'binT',projParam.bin, 'overwrite',projParam.overwrite);
                        end
                    end
                end
