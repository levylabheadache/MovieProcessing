...
% A class to write SBX files, which keeps track of the current position
    ...
    methods
        ...
        function write(obj, data)
            ...
            % If data is 1-color, should be squeezed to   [height, width] or [height, width, length]
            % If data is 2-color, should be squeed to    [2, height, width] or [2, height, width, length]
            ...
            if size(data, 1) == obj.info.nchan  && obj.info.sz(1) == size(data, 2) && obj.info.sz(2) == size(data, 3)  && size(data, 4) + obj.curframe - 1 <= obj.info.nframes % && ndims(data) > 3 % ndims(data) == 4 && 
                if ~isa(data, 'uint16')
