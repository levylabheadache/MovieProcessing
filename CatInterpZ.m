function ExptInterpZ(sbxPath, sbxInfo, varargin)
IP = inputParser;
...
addParameter(IP, 'regParam.binXY', 4, @isnumeric ) %
addParameter(IP, 'regParam.edges',[0,0,0,0], @isnumeric); % [left, right, top, bottom]
...
regParam.refChan = IP.Results.refChan;
...
if ~exist(zinterpPath,'file') || overwrite
    ...
    ref_vol = WriteSbxProjection(sbxPath, sbxInfo, 'firstScan',regParam.refScan(1), 'Nscan',numel(regParam.refScan), 'type','zref', 'chan',regParam.refChan, 'verbose',true, 'monochrome',true, 'overwrite',false); % , 'regParam.edges',regParam.edges , 'regParam.binXY',regParam.binXY
    ...
    for scan = 1:sbxInfo.totScan
        ...
        %curr_scan = WriteSbxProjection(sbxPath, sbxInfo, 'firstScan',scan, 'Nscan',1, 'chan',regParam.refChan, 'verbose',false, 'monochrome',false, 'regParam.edges',regParam.edges, 'regParam.binXY',regParam.binXY, 'overwrite',false); %
    ...
    save(zinterpPath, 'RS','CS','ZS','regParam.binXY', '-mat'); % 'RS1','CS1','ZS1','RS2','CS2','RS3','CS3','RS_chunk','CS_chunk','ZS_chunk','regParam.binXY'
    delete(w);
else
